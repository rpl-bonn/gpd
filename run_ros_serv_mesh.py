...
def main():
    ...
    if event_triggered and top_grasps:
        ...
        try:
            ...
            # Also save a mesh-based visualization file
            mesh_output_path = "/workspace/grasp_meshes_for_rerun.ply"
            create_mesh_visualization(merged_cloud, top_grasps, mesh_output_path)
            rospy.loginfo("Saved mesh-based grasp visualization to %s" % mesh_output_path)
        except Exception as e:
            rospy.logerr("Failed to save visualization files: %s" % e)
...
def create_mesh_visualization(point_cloud, grasps, output_path):
    """
    Create a mesh-based visualization of the gripper hands using Python 2 compatible code.
    This saves a PLY file with both point clouds and triangle meshes representing the grippers.
    
    Args:
        point_cloud: Original point cloud
        grasps: List of grasp configurations
        output_path: Path to save the PLY mesh file
    """
    try:
        ...
        # Create PLY file from scratch
        with open(output_path, 'w') as f:
            # Write PLY header
            f.write("ply\n")
            f.write("format ascii 1.0\n")
            
            # Count number of scene points
            cloud_points = point_cloud.to_list()
            num_cloud_points = len(cloud_points)
            
            # Calculate total points and faces
            # For each grasp we create:
            # - 2 fingers (8 vertices, 12 triangles each)
            # - 1 palm/base (8 vertices, 12 triangles)
            # - 1 approach vector (8 vertices, 12 triangles)
            vertices_per_grasp = 32  # 8 vertices * 4 components
            faces_per_grasp = 48     # 12 triangles * 4 components
            
            total_vertices = num_cloud_points + (vertices_per_grasp * len(grasps))
            total_faces = faces_per_grasp * len(grasps)
            ...
            # Write original cloud points (with white color)
            has_rgb = len(cloud_points[0]) >= 4 if cloud_points else False
            for p in cloud_points:
                x, y, z = p[:3]
                if has_rgb and len(p) >= 4:
            ...
            # Keep track of the vertex index offset for faces
            vertex_offset = num_cloud_points
            
            # Create mesh data for each grasp
            vertex_indices = []  # Will store all face definitions
            ...
            for grasp_idx, grasp in enumerate(grasps):
                ...
                # Color based on score (from blue=low to red=high)
                norm_score = (score - min_score) / score_range
                r_val = int(255 * norm_score)
                g_val = 0
                b_val = int(255 * (1 - norm_score))
                
                # Slightly different colors for different parts
                color_fingers = (r_val, g_val, b_val)
                color_base = (max(0, r_val-50), g_val, min(255, b_val+50))
                color_approach = (255, 50, 50)
                ...
                # --- Create LEFT FINGER ---
                left_finger_vertices = []
                # Create 8 vertices for a cuboid
                for dx in [-0.5, 0.5]:  # Along finger length
                    for dy in [-0.5, 0.5]:  # Along finger width
                        for dz in [-0.5, 0.5]:  # Along finger height
                            local_offset = np.array([
                                dx * hand_depth,
                                dy * finger_width,
                                dz * hand_height
                            ])
                            global_pos = left_bottom + rot_matrix.dot(local_offset)
                            left_finger_vertices.append(global_pos)
                            # Write vertex to PLY
                            f.write("%f %f %f %d %d %d\n" % 
                                   (global_pos[0], global_pos[1], global_pos[2],
                                   color_fingers[0], color_fingers[1], color_fingers[2]))
                
                # Define the 12 triangular faces for the cuboid using proper winding order
                # Each cuboid has 6 faces, each face has 2 triangles
                left_finger_faces = [
                    # Front face
                    [0, 1, 3], [0, 3, 2],
                    # Back face
                ...
                # Store faces with proper vertex offsets
                for face in left_finger_faces:
                    vertex_indices.append([3, 
                                          face[0] + vertex_offset, 
                                          face[1] + vertex_offset, 
                                          face[2] + vertex_offset])
                
                # Update vertex offset for the next component
                vertex_offset += 8
                
                # --- Create RIGHT FINGER ---
                right_finger_vertices = []
                for dx in [-0.5, 0.5]:
                    for dy in [-0.5, 0.5]:
                        for dz in [-0.5, 0.5]:
                            local_offset = np.array([
                            ...
                            global_pos = right_bottom + rot_matrix.dot(local_offset)
                            right_finger_vertices.append(global_pos)
                            f.write("%f %f %f %d %d %d\n" % 
                                   (global_pos[0], global_pos[1], global_pos[2],
                                   color_fingers[0], color_fingers[1], color_fingers[2]))
                
                # Define the 12 triangular faces (same pattern as left finger)
                right_finger_faces = [
                    [0, 1, 3], [0, 3, 2],
                    [4, 6, 7], [4, 7, 5],
                    [0, 2, 6], [0, 6, 4],
                ...
                for face in right_finger_faces:
                    vertex_indices.append([3, 
                                          face[0] + vertex_offset, 
                                          face[1] + vertex_offset, 
                                          face[2] + vertex_offset])
                ...
                for dx in [-0.5, 0.5]:
                    for dy in [-0.5, 0.5]:
                        for dz in [-0.5, 0.5]:
                            ...
                            global_pos = base_center + rot_matrix.dot(local_offset)
                            f.write("%f %f %f %d %d %d\n" % 
                                   (global_pos[0], global_pos[1], global_pos[2],
                                   color_base[0], color_base[1], color_base[2]))
                
                # Define the 12 triangular faces for base (same pattern)
                base_faces = [
                    [0, 1, 3], [0, 3, 2],
                    [4, 6, 7], [4, 7, 5],
                    [0, 2, 6], [0, 6, 4],
                ...
                for dx in [-0.5, 0.5]:
                    for dy in [-0.5, 0.5]:
                        for dz in [-0.5, 0.5]:
                            # Make the approach vector narrower at the tip
                            taper_factor = 0.6 if dx < 0 else 1.0
                            local_offset = np.array([
                                dx * approach_depth,
                                dy * finger_width * taper_factor,
                                dz * 0.5 * hand_height * taper_factor
                            ])
                            global_pos = approach_center + rot_matrix.dot(local_offset)
                            f.write("%f %f %f %d %d %d\n" % 
                                   (global_pos[0], global_pos[1], global_pos[2],
                                   color_approach[0], color_approach[1], color_approach[2]))
                
                # Define the 12 triangular faces for approach vector (same pattern)
                approach_faces = [
                    [0, 1, 3], [0, 3, 2],
                    [4, 6, 7], [4, 7, 5],
                    [0, 2, 6], [0, 6, 4],
        ...
        # Also create a separate file with just the gripper meshes
        grippers_only_path = output_path.replace(".ply", "_grippers_only.ply")
        with open(grippers_only_path, 'w') as f:
            # Write PLY header for grippers only
            f.write("ply\n")
            f.write("format ascii 1.0\n")
            
            # Calculate vertices and faces (same as before, but without scene points)
            total_gripper_vertices = vertices_per_grasp * len(grasps)
            
            f.write("element vertex %d\n" % total_gripper_vertices)
            f.write("property float x\n")
            f.write("property float y\n")
            f.write("property float z\n")
            f.write("property uchar red\n")
            ...
            for grasp_idx, grasp in enumerate(grasps):
                # Extract grasp parameters (same as before)
                pos = np.array([grasp.position.x, grasp.position.y, grasp.position.z])
                approach = np.array([grasp.approach.x, grasp.approach.y, grasp.approach.z])
                binormal = np.array([grasp.binormal.x, grasp.binormal.y, grasp.binormal.z])
                axis = np.array([grasp.axis.x, grasp.axis.y, grasp.axis.z])
        ...
        rospy.loginfo("Created mesh files: %s and %s" % (output_path, grippers_only_path))
        return True
    except ImportError as e:
        rospy.logwarn("Required library not found for mesh generation: %s" % e)
        return False
    except Exception as e:
        rospy.logerr("Error creating mesh visualization: %s" % e)
        return False
